...
<style>
  ...
  /* Global typography and overflow scale */
  html, body, [class*="css"], .stApp {{
      font-family: system-ui, Inter, 'Segoe UI', sans-serif !important;
      background-color: {PAL_BG};
      color: {PAL_INK};
  }}

  /* Absolutely NO text truncation or ellipses */
  p, span, div, h1, h2, h3, h4, h5, h6, table, td, th {{
      white-space: normal !important;
      text-overflow: clip !important;
      overflow-wrap: break-word !important;
  }}

  /* Minimalist dark slate sidebar navigation */
...
def main() -> None:
    ...
    if page == "📊 Dashboard Overview":
        ...
        # API submit simulator inside workspace
        render_live_submit()
        
        # Operations workspace queue + detail action panel
        filtered_view, selected_row = render_live_feed(df)
        if filtered_view.empty:
            ...
            st.info("💡 Select a complaint row in the table above to open its operational workspace and details.")
            
        st.divider()
        with st.expander("📝 Review Recent AI-Drafted Responses (Last 20)"):
            ...
            for _, r in drafted.iterrows():
                with st.container(border=True):
                    st.markdown(f"**{r['id']} &bull; {r['customer_name']} &bull; {r['category']} ({r['severity']})**")
                    st.caption(f"Channel: {r['channel']}  |  Language: {r['language']}  |  SLA due: {pd.to_datetime(r['sla_due_date']).date()}")
                    st.markdown("**Original complaint:**")
